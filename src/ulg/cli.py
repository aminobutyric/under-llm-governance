...
def build_parser() -> argparse.ArgumentParser:
    ...
    resume = subparsers.add_parser(
        "resume", help="resume a retained durable coding task"
    )
    ...
    resume.add_argument("--state-dir", type=Path, default=None)

    discard = subparsers.add_parser(
        "discard", help="destroy a retained task workspace and revoke its grants"
    )
    discard.add_argument("task_id", type=UUID)
    discard.add_argument("--config", type=Path, default=None)
    discard.add_argument("--state-dir", type=Path, default=None)
    return parser
